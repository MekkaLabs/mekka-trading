...
Última atualização: Story 029a (Safety Net). 29 stories entregues,
108 testes Python coletados (14 fase 1 + 20 fase 2 + 6 fase 3 + 7 fase 4
+ 14 fase 5 contracts + 13 fase 5 safety-net + dashboard replay).
...
5. **A última story em `docs/stories/`** (numérica). Hoje: `story-029a`.
...
| Pytest                   | 108 verdes (phase1-4 + phase5 contracts + safety-net) |
| Dashboard web            | aiohttp + websocket, em `src/dashboard/`            |
| Safety Net (029a)        | total-capital cap + 2 consecutive-event breakers + scripts/kill.sh ✅ |
| Telegram bot rico        | pendente                                            |
| Wolverine monitor real   | pendente (cycle ainda é heartbeat)                  |
| Daily PnL writer         | ativo (DailyPnLWriter alimenta `get_today_drawdown_pct`) |
| Hyperliquid SDK testnet  | smoke test manual pendente (bloqueia 029)           |
...
| - | -------------------- | ------- | ----- | ---------------------------- |
| 029 | Wolverine          | Alto    | Baixo | 027 (drawdown), 028 (clean contracts), 029a (safety net) |
