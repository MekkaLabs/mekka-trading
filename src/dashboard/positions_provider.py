...
data (via the configured exchange) or to keep returning the stub shape.
...
async def fetch_positions(
    ...
    # ── Bybit / Binance via CCXT ──────────────────────────────────────
    if settings.active_exchange in ("bybit", "binance"):
        return await _fetch_ccxt_positions(settings.active_exchange)
