    ...
    async def _get_exchange(self) -> Any:
        ...
        for ex_id in all_attempts:
            try:
                ...
                # set_sandbox_mode() helper. Wrapped in try/except so a
                # failed sandbox call or load_markets does not leak the
                # half-initialised aiohttp session (codex hardening).
                try:
                    try:
