...
# Recipe IMP-6b9e6c1ec5d9 — Refatorar nick_fury.py (3003 linhas)
...
`src/agents/nick_fury.py` tem 3003 linhas — acima do limite de 1500. Arquivos grandes concentram risco, dificultam revisão e testes. Quebrar em módulos coesos por responsabilidade.
...
src/agents/nick_fury.py: 3003 linhas (limite 1500, enorme ≥4000).
...
1. Ler o brief original em `docs/improvement-queue/IMP-6b9e6c1ec5d9.md`.
2. Confirmar escopo e blast radius (≤5 arquivos, ≤500 linhas recomendado).
3. Criar branch local: `git checkout -b imp/IMP-6b9e6c1ec5d9`
...
6. Commit com tag IMP no subject:
   ```bash
   git commit -m "[IMP-6b9e6c1ec5d9] Refatorar nick_fury.py (3003 linhas)"
   ```
...
- [ ] Commit subject contém `[IMP-6b9e6c1ec5d9]`

## Notas relacionadas

- Brief original: [[IMP-6b9e6c1ec5d9]]
