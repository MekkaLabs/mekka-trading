...
# Recipe IMP-5a242bf8e4a7 — Refatorar iron_man.py (1785 linhas)
...
`src/agents/iron_man.py` tem 1785 linhas — acima do limite de 1500. Arquivos grandes concentram risco, dificultam revisão e testes. Quebrar em módulos coesos por responsabilidade.
...
src/agents/iron_man.py: 1785 linhas (limite 1500, enorme ≥4000).
...
1. Ler o brief original em `docs/improvement-queue/IMP-5a242bf8e4a7.md`.
2. Confirmar escopo e blast radius (≤5 arquivos, ≤500 linhas recomendado).
3. Criar branch local: `git checkout -b imp/IMP-5a242bf8e4a7`
...
6. Commit com tag IMP no subject:
   ```bash
   git commit -m "[IMP-5a242bf8e4a7] Refatorar iron_man.py (1785 linhas)"
   ```
...
- [ ] Commit subject contém `[IMP-5a242bf8e4a7]`

## Notas relacionadas

- Brief original: [[IMP-5a242bf8e4a7]]
